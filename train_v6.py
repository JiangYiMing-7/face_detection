"""使用更强数据增强配置训练 v6 自定义 Viola-Jones cascade。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--augment", action="store_true", help="对正样本做翻转、亮度、Gamma 和噪声增强（合计×7）。")
...
def augment_windows(windows: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    """数据增强：水平翻转 + 亮度扰动 + Gamma 变换 + 高斯噪声，正样本量合计 ×7。

    - 水平翻转：模拟不同朝向的正脸（左右对称）
    - 亮度扰动：×0.82 和 ×1.18，模拟不同光照条件
    - Gamma 变换：γ=1.5（暗光）和 γ=0.7（过曝），模拟极端光照
    - 高斯噪声：σ=10，模拟传感器噪声，提升鲁棒性
    ...
    gamma_low  = np.clip(np.power(windows / 255.0, 1.5) * 255.0, 0.0, 255.0).astype(np.float32)
    gamma_high = np.clip(np.power(windows / 255.0, 0.7) * 255.0, 0.0, 255.0).astype(np.float32)
    noisy      = np.clip(windows + rng.normal(0, 10, windows.shape), 0.0, 255.0).astype(np.float32)
    return np.concatenate([windows, flipped, darker, brighter, gamma_low, gamma_high, noisy], axis=0)
...
def main() -> None:
    ...
    if args.augment:
        ...
        print(f"{len(positives)} (flip + brightness + gamma + noise ×7)")
