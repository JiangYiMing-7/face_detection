...
人脸检测是人脸识别、表情分析等后续任务的前提。Viola-Jones (CVPR 2001) 通过积分图、AdaBoost 特征选择和级联分类器三项技术，首次实现了实时人脸检测，至今仍是理解特征工程和集成学习的经典案例。

本项目的目标是完全从零实现 Viola-Jones 人脸检测算法（不使用任何深度学习框架），在实现过程中深入理解各环节的原理和工程细节，并将自实现的检测器与 OpenCV 预训练的 Haar Cascade 模型进行对比评估。

### 1.2 项目目标

1. 从零实现 Viola-Jones 算法：积分图、Haar-like 特征、AdaBoost、级联分类器、多尺度滑动窗口检测、NMS 等全部核心模块
...
这里有一个设计选择：我们直接在原始像素值（0-255）上训练，没有做方差归一化。这样训练流程更简单，从后面的实验看，只要配合 `min_neighbors`、`score_threshold` 等后处理，精度并不受影响。
...
这一节介绍我们手写实现所依据的核心算法。为了不让公式停留在纸面上，下面每个公式都标出了它在第 5 节里对应的代码模块。
...
在 $24 \times 24$ 检测窗口内完整枚举会产生超过 160,000 个候选特征。考虑训练时间，本项目使用固定随机种子抽取 20,000 个特征，并把 `feature_count` 保存到模型 JSON 中，保证训练和推理时特征集合一致。

### 4.4 AdaBoost：从大量 Haar 特征中选择少量有效弱分类器
...
*图 1：项目整体框架图，展示数据资产、脚本入口、核心算法模块以及从训练到评估、演示的端到端闭环。*
...
级联的拒绝逻辑很直接：只要某一级分数低于阈值就立即返回"非人脸"，不再进入后续 stage。
...
这里的关键是用 `alive` 掩码做逐级剪枝：被前面 stage 拒绝的窗口不再参与后续计算，再配合 `values_at` 的向量化特征计算，Python 实现也能跑到可用的速度。
...
CLAHE 对图像做局部对比度增强，在光照不均匀时使 Haar 特征响应更稳定。实验效果（LFW 严格操作点，详见 8.4）：
- v3（较弱模型）+ CLAHE：F1 0.919→0.933，误报减少（CLAHE 有益）
- v6/v6.1（较强模型）+ CLAHE：F1 反而下降约 0.02，误报增多（CLAHE 放大背景噪声，有害）

因此 CLAHE 仅作为可选项保留，最终部署的 v6.1 默认关闭 CLAHE。这是一个"预处理增强需与模型能力匹配"的反直觉发现。
...
为此我们在 `sliding_window.py` 中设计了自定义的 `_cluster_weighted_merge` 算法。下面摘录的是项目中的真实实现片段：
...
策略 B（同时约束 FP 率）：在保证检测率的基础上将阈值提高到负样本分数的第 50% 分位数，使每级至少拒绝 50% 负样本。更接近论文设计，但实验发现过于激进的阈值会减少重叠检测框数量，导致 `min_neighbors` 后处理失效（详见 8.3.4 节 v4 模型的教训）。

策略 B 中还设置了 `safe_upper = np.quantile(scores, 0.05)` 作为阈值上限，保证检测率不低于 95%。

### 6.4 自适应级联训练

我们还实现了自适应级联训练模式：不预先指定每级弱分类器数量，而是动态增加，直到当前 stage 的 FP 率降到目标值以下。
...
从 v6（×4，12000样本）到 v6.1（×7，21000样本），级联结构保持不变（10 级 / 525 弱），仅训练数据多样性增加。在 LFW 上两者基本持平（差异在统计噪声内，见 8.4），×7 增强带来的 gamma/噪声鲁棒性主要在光照复杂的实时摄像头场景中体现。

![数据增强消融结果](训练数据与可视化/plots/augmentation_ablation.png)

*图 7：数据增强策略的消融结果。v6.1 在 ×7 增强下进一步消除了最后的误报，说明 gamma 校正与噪声扰动对泛化有帮助。*
...
4. max_lost=2：连续 2 帧未匹配的 track 被删除，减少"幽灵框"。
...
在 `min_size=100` 这个常规操作点上，三个自实现模型在 LFW 上都达到了 F1=1.000，既没有漏检也没有误报，基本到了这个测试集的上限。LFW 大多是正面、近距离、单人脸的照片，把搜索窗口限制在 100 像素以上之后几乎不会产生误检，所以三个模型之间区分不开。OpenCV 预训练模型漏掉了 8 张（大多是侧偏或低对比度的人脸），F1=0.980。

**(2) 严格操作点（`min_size=60`，更多候选窗、更考验对误检的抑制）**

为了把模型之间的真实差距暴露出来，我们把 `min_size` 降到 60。此时图像金字塔层数和候选窗口数量大幅增加，对"非人脸窗口的拒绝能力"提出了更高要求：
...
这组对比就能看出模型容量的作用了：v3（8 级 / 230 弱）在严格操作点上误报 28 个，v6/v6.1（10 级 / 525 弱）只有 13~15 个，F1 高出约 0.04~0.05。v6 和 v6.1 在 LFW 上基本打平（0.964 vs 0.959，差异在波动范围内），这也在意料之中：两者级联结构完全一样（都是 10 级 / 525 弱），区别只在训练时的数据增强（×4 和 ×7）。×7 增强多出来的 gamma 校正和高斯噪声主要是提升对光照、噪声的鲁棒性，在 LFW 这种规整的正脸照片上看不出差别，要到光照更复杂的实时摄像头里才有用。

以上数字都可以用 `evaluate.py` 复现（命令见 8.1 节末尾）：常规操作点对应 `results/eval_lfw_{opencv,v3,v6,v61}/`，严格操作点对应 `results/eval_lfw_strict_{v3,v6,v61}/`。
...
2. 小人脸：Caltech 中部分人脸很小，而我们为了控制误报把 `min_size` 设成了较大的 120
3. 级联层数：OpenCV 预训练模型有 20 层以上级联，我们只有 8 层，对难例的区分能力较弱
4. 训练数据量：OpenCV 模型在数万张图片上训练，我们只用了 1000 张正样本
...
精确率只有 3.3%（输出了 462 个误检框），说明有大量非人脸窗口通过了级联；同时高分误检框还压制了真正的人脸框，使 Recall 也只有 8%。
...
解决方案：我们没有去修改阈值策略（实验表明 FP 约束反而有负面效果，见 8.3.4），而是在后处理阶段引入 `min_neighbors` 参数（对应 `_cluster_weighted_merge` 的 `min_count`）：
...
我们尝试训练了 v4 模型（15 级 / 1210 弱分类器），启用了 FP 率约束（`--fp-constraint`）。训练时的日志看起来非常好：

```
Stage 1:  FPR≈48%      ← 接近 50% 目标
Stage 15: 累积 FP 率 <1%  ← 看起来很棒
```
...
v4 不仅没有比 v3 更好，反而在常规操作点上**漏检明显增多**（FN 从 0 增加到 26）。
...
教训：级联本身的 FP 率和最终检测精度不是简单的正相关。当后处理依赖足够的重叠检测框时，过于激进的阈值反而有害。这也是为什么最终版本只约束检测率、不约束 FP 率。
...
这组对比清楚地展示了 FP 约束的负面效果：v4 用了 15 级、1210 个弱分类器（远多于 v3 的 8 级/230 弱），级联本身的 FP 率也更低，但最终检测 F1 反而更差。原因是过于严格的级联阈值减少了同一人脸的重叠检测框数量，使其无法满足 `min_neighbors` 的要求而被后处理误删。
...
分析：
- v3 到 v6 的提升是实打实的：正样本从 1000 增到 3000，模型容量从 8 级/230 弱增到 10 级/525 弱，严格操作点上的误报从 28 降到 13，F1 提升约 0.045。
- v6 和 v6.1 在 LFW 上基本持平（F1 0.964 vs 0.959，差异在波动范围内）。两者级联结构完全一样（都是 10 级 / 525 弱，用了固定的 `--stage-sizes`），区别只在数据增强 ×4 和 ×7。gamma 校正和高斯噪声带来的鲁棒性，在 LFW 这种规整正脸集上看不出来，主要是在光照、噪声更复杂的实时摄像头场景里才有意义（见第 9 节）。
- 这件事也说明，光看一个简单数据集的指标不足以判断增强策略好不好，还得在更接近真实使用条件的场景里验证。
...
这里有一个有点反直觉的结果：CLAHE 对弱模型有帮助，对强模型反而是负担。
...
14.4 FPS 低于 OpenCV 的 30+ FPS（C++ + SIMD），但对 Python 实现来说已经可用，画面流畅。

性能瓶颈：
- 主要耗时在滑动窗口的特征计算上
- `min_size` 从 30 提高到 80，FPS 从 ~5 提升到 ~14
...
3. 找到问题所在：`choose_stage_threshold` 只约束检测率、不约束 FP 率，结果每级只拒绝了 2~7% 的负样本
...
**现象**：有同学问"怎么把那个东西也识别成人脸了"，其实是摄像头本身的圆形边框被检测成了人脸。

**原因**：`min_size=30` 时会在很小的尺度上滑动，产生大量小窗口，摄像头边框在小尺度下正好和某些 Haar 特征对上了。

**解决方案**：把 `min_size` 从 30 提高到 80。正常距离（50cm~2m）下人脸远大于 80 像素，不会漏检；代价是很远处的小人脸会被跳过，这是速度和精度之间的取舍。
...
**解决方案**：实现 TrackingBoxSmoother，用指数移动平均平滑检测框坐标（详见第 7.2 节）。

### 9.5 关键问题5：v6 模型实时检测的特殊挑战
...
除了基础的级联训练，我们还试了 Hard Negative Mining、FP 率约束、自适应级联训练等策略，并用对比实验看它们到底有没有用。其中"加 FP 约束反而把最终精度做低了"这个结论有点出乎意料，原因是它和 `min_neighbors` 后处理之间的相互作用（详见 8.3.4 节）。
...
该算法完全独立于 OpenCV，在实时摄像头中效果优于 groupRectangles。

### 10.5 LFW 评估结果

v6.1 模型在 LFW 200 张测试集的常规操作点（min_size=100）上达到 F1=1.000，比 OpenCV 预训练 Haar Cascade 的 F1=0.980 略高（OpenCV 漏检了 8 张侧偏或低对比度的人脸）。这里要说清楚：LFW 规模小、又以正面人脸为主，在这个操作点上三个自实现模型都饱和到了 1.000，只能体现受控场景下的检测能力；模型之间的真实差距要到严格操作点（min_size=60）才看得出来（见 8.2 节），并不代表复杂场景下的泛化能力。

实现路径：3000 正样本 + ×7 增强 + 10 级级联 + CLAHE 预处理 + `min_neighbors=10` + `_cluster_weighted_merge`。
...
2. 学会了在 Python 中通过 NumPy 向量化将算法优化到实时可用
3. 理解了后处理参数（`min_neighbors`、`score_threshold`）和级联阈值之间是怎么相互影响的，这一点只看论文很难体会到
4. 面对 3.3% 的 Precision 时，通过编写审计工具定位到根因，而不是简单调参
5. 针对 OpenCV API 的局限，独立设计了 _cluster_weighted_merge 和 TrackingBoxSmoother

### 11.2 局限性

1. 仅支持正面人脸：Haar 特征和训练数据都针对正面人脸，侧脸、遮挡、倾斜场景检测能力有限（Caltech F1=0.553）
2. 级联每级 FPR 偏高：每级只拒绝 15~40% 的负样本（论文期望 50%），要靠 min_neighbors 后处理来补。说到底是级联本身没学透，靠后处理在兜底
