...
为保证项目提交到其他机器后仍可运行，当前版本将评估、训练、演示和辅助调试脚本中的默认路径统一为“项目根目录 + 相对路径”的解析方式，不依赖开发机上的绝对目录。仓库中已包含 `data/test_lfw/` 测试图片、`data/test_lfw/annotations.json` 标注文件和 `models/custom_cascade_v6_full_1.json` 模型文件，因此在安装 `requirements.txt` 后，可直接通过以下命令复现核心结果：
...
其中 `evaluate.py` 默认使用 v6.1 模型和 LFW 200 张测试子集；若需要使用本地额外数据集，例如 LFW 增强包或 Caltech-101 原始数据，则通过 `LFW_TAR`、`CALTECH_ROOT` 环境变量或命令行参数传入。
...
主要检测参数如下：LFW 常规操作点使用 `min_size=100, min_neighbors=5`；严格操作点使用 `min_size=60, min_neighbors=5`；Caltech 上根据模型能力调整 `score_threshold` 和 `min_neighbors`；实时演示中常用 `scale_factor=1.1, min_size=80, window_step=4, min_neighbors=6`，在 LFW 实测中是兼顾检测框贴合度、误检控制和帧率的最优操作点（`scale_factor` 从 1.2 收紧到 1.1 后精确率由 0.94 提升到 1.00）。
